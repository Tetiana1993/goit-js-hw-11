...
`)}function h(r){const o=d(r);l.gallery.insertAdjacentHTML("afterbegin",o)}const a=new p(".gallery a",{captions:!0,captionSelector:"img",captionPosition:"bottom",captionsData:"alt",captionDelay:250,captionType:"attr"});a.on("show.simplelightbox");a.refresh();const l={formEl:document.querySelector(".form"),inputEl:document.querySelector(".saerch-img"),listEl:document.querySelector(".gallery"),spanEl:document.querySelector(".loader")};l.spanEl.style.display="none";l.formEl.addEventListener("submit",y);function y(r){r.preventDefault();const o=r.target.elements.text.value.trim();if(l.spanEl.style.display="block",!o){f();return}m(o).then(n=>{n.hits.length===0?(g(),l.listEl.innerHTML=""):(h(n),a.on("show.simplelightbox"),a.refresh()),l.loader.style.display="none"}).catch(n=>console.log(n)),r.target.reset()}
